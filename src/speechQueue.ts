...
export class SpeechQueue {
  ...
  constructor(play: (text: string, signal: AbortSignal) => Promise<void>) { this.play = play; }
  ...
  say(text: string, interrupt = true): Promise<void> {
    ...
    this.tail = this.tail.then(async () => {
      ...
      try { await this.play(text, controller.signal); }
      finally { if (this.active === controller) this.active = null; }
    }).catch(() => { /* Speech failure never prevents the next game action. */ });
    return this.tail;
  }
}

export function speechParts(text: string): string[] {
  const normalized = text.replace(/(\d+)\/(\d+)/g, '$1 over $2')
    .replace(/−/g, ' minus ').replace(/×/g, ' times ').replace(/÷/g, ' divided by ')
    .replace(/\+/g, ' plus ').replace(/=/g, ' equals ').replace(/\s+/g, ' ').trim();
  ...
  return normalized.split(/(?<=[.!?])\s+/).flatMap(sentence => {
    ...
    for (const word of sentence.split(' ')) {
      if (chunk && chunk.length + word.length > 180) { chunks.push(chunk); chunk = ''; }
      chunk += `${chunk ? ' ' : ''}${word}`;
    }
