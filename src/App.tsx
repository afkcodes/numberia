...
import { Character, EmptyState, Logo, Modal, Stars } from './components';
import { playSound, stopSpeaking } from './audio';
import GradePicker from './GradePicker';
import WorldPicker from './WorldPicker';
import Clubhouse from './Clubhouse';
import { chooseRoomItem } from './clubhouse';
import { rememberDiscovery, recommendedSkill, skillMemory } from './learning';
...
type Page = 'adventure' | 'practice' | 'backpack' | 'progress' | 'clubhouse';
type Overlay = 'profile' | 'parents' | 'worlds' | 'help' | 'grades' | 'daily' | null;
const navigation = [
  { id: 'adventure' as Page, label: 'My adventure', icon: Map },
  { id: 'practice' as Page, label: 'Practice camp', icon: Tent },
  { id: 'clubhouse' as Page, label: 'My clubhouse', icon: Home },
  { id: 'backpack' as Page, label: 'My backpack', icon: Backpack },
  { id: 'progress' as Page, label: 'My progress', icon: ChartNoAxesCombined },
];
...
function DailyQuests({
  ...
  const today = dateKey();
  const runs = save.runs.filter((r) => r.date === today);
  const tasks = [
    {
      key: `${today}:mission`,
      icon: FlagIcon,
      ...
      progress: Math.min(1, runs.filter((r) => !r.practice).length),
      total: 1,
      reward: 10,
      skill: undefined,
    },
...
function AdventureMap({
  ...
  useEffect(() => setSelected(current), [current, save.grade]);
...
function Practice({ save, onStart }: { save: Save; onStart: (skill: Skill) => void }) {
...
function ProgressPage({ save, onAdventure }: { save: Save; onAdventure: () => void }) {
...
export default function App() {
  const [save, setSave] = useState<Save>(() => {
    try {
      ...
      return { ...defaultSave, runs: [], claimed: [] };
    }
  });
  const [page, setPage] = useState<Page>('adventure');
  const [overlay, setOverlay] = useState<Overlay>(null);
  const [quest, setQuest] = useState<{ id: string; index: number; practiceSkill?: Skill } | null>(
    null,
  );
  ...
  const dailyClaim = (key: string) =>
    setSave((s) =>
      s.claimed.includes(key)
        ? s
        : { ...s, gems: s.gems + (key.endsWith(':stars') ? 15 : 10), claimed: [...s.claimed, key] },
    );
  const complete = (run: Run) => setSave((s) => recordRun(s, run));
  const exportProgress = () => {
    const data = new Blob([JSON.stringify(save, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(data);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'numberia-progress.json';
    a.click();
  ...
  return (
    <>
        ...
        <div className="main-shell">
          ...
          <main id="main-content" className="main-content">
              ...
              <>
                ...
                <section className="welcome-banner">
                  <div className="welcome-copy">
                    ...
                    <button className="welcome-link" onClick={() => start()}>
            ...
            {page === 'clubhouse' && (
              <Clubhouse
                save={save}
                onChoose={(id) => setSave((s) => chooseRoomItem(s, id))}
                onName={(petName) =>
                  setSave((s) => ({ ...s, clubhouse: { ...s.clubhouse, petName } }))
                }
                onAdventure={() => start()}
              />
            )}
            {page === 'backpack' && (
              <BackpackPage
                save={save}
                onCompanion={(companion) => setSave((s) => ({ ...s, companion }))}
      ...
      {quest && (
        ...
        >
          <Quest
            ...
            onComplete={complete}
            onLearn={(observation) => setSave((s) => rememberDiscovery(s, observation))}
            onClubhouse={() => {
              setQuest(null);
              goTo('clubhouse');
            }}
            onSoundChange={() => setSave((s) => ({ ...s, sound: !s.sound }))}
            onContinue={() => {
      ...
      {overlay === 'daily' && (
        <Modal
          title="Today’s little quests"
          onClose={() => setOverlay(null)}
          className="daily-quests-modal"
        >
          <DailyQuests
            save={save}
            onStart={(skill) => {
              setOverlay(null);
              start(nextMission(save), skill);
            }}
            onClaim={dailyClaim}
          />
        </Modal>
      ...
      {overlay === 'grades' && (
        <Modal
          title="Choose your learning grade"
          onClose={() => setOverlay(null)}
      ...
      {overlay === 'profile' && (
        ...
        >
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              setSave((s) => ({ ...s, name: nameDraft.trim() || 'Explorer' }));
              setOverlay(null);
            }}
          >
            <label>
            ...
            <GradePicker
              compact
              grade={save.grade}
              onSelect={(grade) => setSave((s) => ({ ...s, grade }))}
            />
      ...
      {overlay === 'help' && (
        <Modal title="A little help" onClose={() => setOverlay(null)}>
          <div className="info-modal">
            ...
            <div className="help-step">
              ...
              <div>
                ...
                <p>
                  Pick your grade, then press “Let’s play” on the adventure map. The first chapter
                  is ready for you.
                </p>
              </div>
            </div>
            <div className="help-step">
              ...
              <div>
                ...
                <p>
                  Drag berries into a basket, build a bridge, or share snacks equally. You can also
                  tap an object, then its basket. Switch to Explore & answer to roam the park with
                  Milo and use the big answer buttons.
                </p>
              </div>
            </div>
            <div className="help-step">
              ...
              <div>
                ...
                <p>
                  Finish five challenges to earn all three stars and a keepsake. Hints and retries
                  never reduce your rewards. New chapters and buddies unlock as you explore.
                </p>
              </div>
            </div>
            <div className="milo-note">
              <Lightbulb size={28} />
              <p>
                Stuck? Ask for a hint. There are no timers, lost lives, or lost stars. Every try is
      ...
      {overlay === 'parents' && (
        <Modal title="Grown-up corner" onClose={() => setOverlay(null)} className="parents-modal">
          ...
          {parentTab === 'overview' ? (
            <>
              ...
              <p className="parent-copy">
                Progress stays in this browser. There are no accounts, ads, real-money purchases,
                public rankings, or time limits. Switching grades keeps each grade’s chapter
                progress.
              </p>
            ...
            <div className="learning-details">
              ...
              <p>
                ...
                stars, 100 XP, and 15 gems; hints and retries never reduce rewards. Learning is
                saved separately for each grade and skill, including between visits. After two
                independent discoveries, the challenge grows; after support, it gently eases.
                Supported facts return after other practice, and every visit begins with a warm-up.
                Decorating the clubhouse uses earned gems only. Practice asks children to retrieve
                an answer, with hints and another try when needed.
              </p>
              <h3>Practice grows with the learner.</h3>
              <p>
                K–2 covers addition and subtraction; grades 3–4 add multiplication and division;
                grade 5 also explores same-denominator fractions and tenths. Grade labels are
                starting points, not a complete curriculum or an assessment.
              </p>
              <h3>Helpful color. Purposeful movement.</h3>
              <p>
                Colors consistently distinguish groups, and each answer has a number and text
                feedback. Movement celebrates restored objects. Reduced-motion preferences are
                respected. We don’t claim a particular color palette or this game has been
              ...
              <p>
                ...
                , and{' '}
                <a
                  href="https://ies.ed.gov/ncee/wwc/practiceguide/26"
                  target="_blank"
                  rel="noreferrer"
                >
