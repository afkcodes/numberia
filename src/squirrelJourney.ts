...
export function squirrelJourney(time: number, distance: number, climbHeight: number) {
  ...
  const pickup = 3.8 + travel,
    turnBack = pickup + 3,
    returnTrip = turnBack + 0.8;
  const tree = returnTrip + travel,
    climb = tree + 0.8,
    home = climb + 3.8;
  ...
  if (t < 3) {
    ...
    rotation = 90 * (1 - (t - 3) / 0.8);
    action = 'landing';
  } else if (t < pickup) {
    x = (distance * (t - 3.8)) / travel;
    gait = 'run';
    action = 'going to nuts';
  } else if (t < turnBack) {
    x = distance;
    carrying = t > pickup + 1.5;
    action = 'picking up';
  } else if (t < returnTrip) {
    x = distance;
    turn = t < turnBack + 0.4 ? 1 : -1;
    carrying = true;
    action = 'turning';
  } else if (t < tree) {
    x = distance * (1 - (t - returnTrip) / travel);
    turn = -1;
    carrying = true;
    gait = 'run';
    action = 'returning';
    ...
    turn = -1;
    carrying = true;
    rotation = (90 * (t - tree)) / 0.8;
    action = 'gripping tree';
  } else if (t < home) {
    const p = (t - climb) / 3.8;
    y = -climbHeight * p;
    rotation = 90;
