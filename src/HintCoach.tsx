    ...
    : skill === 'addition'
      ...
      : skill === 'subtraction'
        ? [
            ...
            a <= 10
              ? `Find ${a} berries. When ${b} go away, count just the berries that are still here.`
              : `We can take away the tens first, then the ones. Small steps make a big problem easier.`,
  ...
  return (
    <aside className="hint-coach" aria-label="Milo’s helping hand">
      <div className="coach-character">
        <Character />
        <span className="coach-heart">
          <Heart size={19} fill="currentColor" />
        </span>
      </div>
      <div className="coach-bubble">
        <button className="coach-close" aria-label="Hide Milo’s help" onClick={onClose}>
          <X size={17} />
        </button>
        <span className="coach-label">
          <Heart size={13} />
          MILO’S GOT YOUR BACK
        </span>
        <p key={step} aria-live="polite">
          {steps[step]}
        </p>
        <div className="coach-actions">
          ...
          <span className="coach-dots" aria-label={`Step ${step + 1} of 3`}>
            {steps.map((_, i) => (
