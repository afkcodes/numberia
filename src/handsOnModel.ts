...
export type PlayModel = {
  kind: 'gather' | 'bridge' | 'groups' | 'share' | 'pieces';
  tokens: PlayToken[];
  targets: number;
  scale: number;
  goal: number;
...
function packets(amount: number, group: number, unit = false): PlayToken[] {
  ...
  const values: number[] = [];
  for (const size of unit ? [1] : [1000, 100, 10, 1])
...
export function playModel(p: Problem): PlayModel {
  ...
  if (p.skill === 'division')
    return { kind: 'share', tokens: packets(p.a, 0, true), targets: p.b, scale, goal: p.answer };
  if (p.skill === 'multiplication')
    return {
      kind: 'groups',
      tokens:
        p.a * p.b > 30
          ? Array.from({ length: p.a }, (_, i) => ({ id: `0-${i}`, value: p.b, group: 0 }))
          : packets(p.a * p.b, 0, true),
      targets: p.a,
      scale,
      goal: p.b,
    };
  ...
  return {
    kind: p.denominator ? 'pieces' : 'gather',
    tokens: [
      ...packets(
        Math.round(p.a * scale),
        0,
        !!p.denominator || (p.skill !== 'decimals' && p.answer <= 20),
      ),
      ...packets(
        Math.round(p.b * scale),
        1,
        !!p.denominator || (p.skill !== 'decimals' && p.answer <= 20),
      ),
    ],
    targets: 1,
    scale,
    goal: Math.round(p.answer * scale),
  };
}
export const targetTotal = (model: PlayModel, placements: Placements, target: number) =>
  model.tokens.reduce((n, t) => n + (placements[t.id] === target ? t.value : 0), 0);
export function completedPlay(model: PlayModel, placements: Placements) {
  ...
  return (
    model.tokens.every((t) => placements[t.id] !== undefined) &&
    Array.from({ length: model.targets }, (_, i) => targetTotal(model, placements, i)).every(
      (n) => n === model.goal,
    )
  );
}
export function demonstrationPlacements(model: PlayModel): Placements {
  ...
  for (const token of model.tokens) {
    ...
    total += token.value;
    if (model.targets > 1 && total === model.goal) {
      target++;
      total = 0;
    }
  }
...
export type BridgePiece = {
  id: string;
  value: number;
  kind: 'existing' | 'added' | 'gap' | 'surplus';
  token?: PlayToken;
  first?: boolean;
};
...
export function bridgePieces(p: Problem, model: PlayModel, placements: Placements): BridgePiece[] {
  const unit = p.a <= 20;
  const existing: BridgePiece[] = packets(p.b, 2, unit).map((t) => ({
    id: `built-${t.id}`,
    value: t.value,
    kind: 'existing',
  }));
  ...
  const placed: BridgePiece[] = Object.keys(placements).flatMap((id) => {
    ...
    added += token.value;
    return [
      { id: token.id, value: token.value, kind: added <= model.goal ? 'added' : 'surplus', token },
    ];
  });
  const gaps: BridgePiece[] = packets(Math.max(0, model.goal - added), 3, unit).map((t, i) => ({
    id: `gap-${t.id}`,
    value: t.value,
    kind: 'gap',
    first: i === 0,
