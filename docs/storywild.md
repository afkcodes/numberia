...
Hosts without inbound WebSockets, such as Vercel, reject the upgrade. The client then keeps the PCM locally and, at **I’m done**, posts it once to `/api/reading/transcribe`. That route opens the same provider socket server-side, commits every 30 seconds to stay under Nari’s 36-second utterance limit, and returns only the joined final transcript. Words are marked after the child finishes rather than while they read. It shares the origin and key checks, allows ten requests per client per minute and two at once, and accepts at most 100 seconds of audio.
